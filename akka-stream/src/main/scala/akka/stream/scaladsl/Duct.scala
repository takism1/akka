...
trait Duct[In, +Out] {
   ...
   */
  def produceTo(consumer: Consumer[Out] @uncheckedVariance, materializer: FlowMaterializer): Consumer[In]
   ...
   */
  def onComplete(callback: Try[Unit] ⇒ Unit, materializer: FlowMaterializer): Consumer[In]
