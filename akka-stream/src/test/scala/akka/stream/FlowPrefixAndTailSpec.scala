...
class FlowPrefixAndTailSpec extends AkkaSpec {
  ...
  "PrefixAndTail" must {
    ...
    "work if size of take is equals to stream size" in {
      ...
      Flow(tail).produceTo(consumer, m)
    ...
    "handle onError when no substream open" in {
      ...
      Flow(producer).prefixAndTail(3).produceTo(consumer, m)
    ...
    "handle onError when substream is open" in {
      ...
      Flow(producer).prefixAndTail(1).produceTo(consumer, m)
      ...
      Flow(tail).produceTo(substreamConsumer, m)
